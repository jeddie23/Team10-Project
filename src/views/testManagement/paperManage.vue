  ...
  <div class="table-classic-wrapper">
    <el-card shadow="always">
      ...
      <el-table ref="multipleTable" v-loading="listLoading" :data="tableData" tooltip-effect="dark" style="width: 100%"
        size="medium" @selection-change="handleSelectionChange" @select="handleSelect">
      ...
      <!-- 新增考试 弹出栏 -->
      <el-dialog title="新增考试" :visible.sync="formVisible" width="35%" class="dialog-form" :before-close="handleClose">
        <el-form ref="newTestForm" :model="newTestForm" :rules="formRules" label-width="100px">
          ...
          <el-form-item label="考试开始时间:" prop="startTime">
            <el-date-picker v-model="newTestForm.startTime" type="datetime" placeholder="选择日期"
              value-format="yyyy-MM-dd HH:mm:ss">
            </el-date-picker>
          </el-form-item>
          <el-form-item label="考试结束时间:" prop="startTime">
            <el-date-picker v-model="newTestForm.endTime" type="datetime" placeholder="选择日期"
              value-format="yyyy-MM-dd HH:mm:ss">
            </el-date-picker>
          </el-form-item>
          <el-form-item>
            <p>已选学生:</p>
            <el-tag v-for="item in selectedTags" :key="item.id" :type="item.type" closable @close="deleteUser(item)">
              {{ item.name }}
            </el-tag>
          </el-form-item>
          <el-form-item>
            <p>所有学生:</p>
            <el-tag v-for="item in allUserTags" :key="item.name" :type="item.type" @click="select(item)">
              {{ item.name }}
            </el-tag>
          </el-form-item>
      ...
      <!-- 查看试卷弹出 -->
      <el-dialog title="新增考试" :visible.sync="paperVisible" width="40%" class="dialog-form" :before-close="cancleTestForm">
        <el-form ref="paperDetail" :model="paperDetail" :rules="formRules" label-width="100px">
          ...
          <el-form-item>
            <el-tag v-for="item in paperDetail.question" :key="item.questionId" type="" effect="plain"
